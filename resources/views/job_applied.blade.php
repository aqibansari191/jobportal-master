                                        ...
                                        @if ($jobApplications->isNotEmpty())
                                            @foreach ($jobApplications as $jobApplication)
                                                <tr class="active">
                                                    ...
                                                    <td>{{ $jobApplication->job->applications->count() }}20 Applications</td>
                                                    <td>
                                                        @if ($jobApplication->job->status == 1)
                                                            <div class="job-status text-capitalize">Active</div>
                                                        @else
                                                            <div class="job-status text-capitalize">Black</div>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        ...
                                                        <div class="action-dots float-end">
                                                            ...
                                                            <ul class="dropdown-menu dropdown-menu-end">
                                                                <li><a class="dropdown-item" href="{{ route("jobDetail",$jobApplication->job->id) }}"> <i class="fa fa-trash" aria-hidden="true"></i> View</a></li>
    ...
    if (confirm("Are you sure you want to remove?")) {
        $.ajax({
            ...
            success: function(response) {
                if (response.status) {
                    window.location.href = '{{ route("mjobApplication") }}';
                } else {
